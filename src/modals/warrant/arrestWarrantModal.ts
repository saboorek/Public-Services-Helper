...
export const arrestWarrantModal = {
    customId: "arrestWarrantModal",
    ...
    async execute(interaction: ModalSubmitInteraction): Promise<void> {
        ...
        try {
            ...
            const narrative = interaction.fields.getTextInputValue('arrestWarrantNarrative');
            const applicant = interaction.fields.getTextInputValue('arrestWarrantApplicant');
            const suspect = interaction.fields.getTextInputValue('arrestWarrantSuspect');
            ...
            if (!config?.warrants?.arrestChannelId) {
            ...
            await WarrantCase.create({
                ...
                type: 'arrest',
                fields: [
                    { name: "🎯 Osoba objęta nakazem aresztowania", value: suspect },
                ]
            });
            ...
            const channelEmbed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: 'Osoba objęta nakazem aresztowania:', value: suspect, inline: true }
                )
                .setTimestamp();
            ...
            const arrestChannel = await interaction.guild!.channels.fetch(config.warrants.arrestChannelId).catch( () => null ) as TextChannel | null;

            if (!arrestChannel) {
                await interaction.editReply({
                    content: '❌ Nie znaleziono kanału dla nakazów aresztowania.'
                });
                return;
            }
            ...
            logger.success(`Arrest warrant złożony przez ${interaction.user.id} w ${interaction.guildId}`);

        } catch (error) {
            logger.error(`Error creating arrest warrant channel in guild ${interaction.guildId}: ${error}`);
            logger.error(`Błąd podczas przetwarzania wniosku o nakaz aresztowania: ${error}`);
            await interaction.editReply({
