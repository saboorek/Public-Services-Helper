...
export const surveillanceWarrantModal = {
    customId: "surveillanceWarrantModal",
    ...
    async execute(interaction: ModalSubmitInteraction): Promise<void> {
        ...
        try {
            const narrative = interaction.fields.getTextInputValue('surveillanceWarrantNarrative');
            const applicant = interaction.fields.getTextInputValue('surveillanceWarrantApplicant');
            const place = interaction.fields.getTextInputValue('surveillanceWarrantPlace');

            const config = await WarrantConfig.findOne({ guildId: interaction.guildId });

            if (!config?.warrants?.categoryId) {
                await interaction.editReply({
                    content: '❌ Kategoria dla nakazów nie została skonfigurowana.'
                });
                return;
            }

            if (!config?.warrants?.surveillanceChannelId) {
                await interaction.editReply({
                    content: '❌ Kanał dla nakazów inwigilacji nie został skonfigurowany.'
                });
                return;
            }
            ...
            await WarrantCase.create({
                ...
                type: 'surveillance',
                fields: [
                    { name: 'Miejsce/mienie/osoby objęte nakazem:', value: place },
                ]
            });
            ...
            const channelEmbed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: 'Miejsce/mienie/osoby objęte nakazem:', value: place, inline: true }
                )
                .setTimestamp();
            ...
            const surveillanceChannel = await interaction.guild!.channels.fetch(config.warrants.surveillanceChannelId).catch( () => null ) as TextChannel | null;

            if (!surveillanceChannel) {
                await interaction.editReply({
                    content: '❌ Nie znaleziono kanału dla nakazów o narzędzia inwigilacji'
                });
                return;
            }
            ...
            const reviewEmbed = new EmbedBuilder()
                .setTitle('🛰️ Wniosek o nakaz na narzędzia inwigilacji')
                .setDescription(`Nowy wniosek o nakaz na narzędzia inwigilacji od ${interaction.user.tag} (${interaction.user.id})`)
                .setColor(EmbedColors.waiting)
                .addFields(
                    { name: 'Narracja dot. Probable Cause oraz dowody:', value: narrative, inline: false },
                    { name: 'Wnioskujący o nakaz, pozycja oraz agencja:', value: applicant, inline: true },
                    { name: 'Miejsce/mienie/osoby objęte nakazem:', value: place, inline: true },
                    { name: 'Kanał wniosku:', value: `<#${channel.id}>`, inline: false }
                )
                .setTimestamp();
            ...
            logger.success(`Surveillance warrant złożony przez ${interaction.user.id} w ${interaction.guildId}`);
        } catch (error){
            logger.error(`Error creating surveillance warrant channel in guild ${interaction.guildId}: ${error}`);
            logger.error(`Błąd podczas przetwarzania wniosku o nakaz na narzędzia inwigilacji: ${error}`);
            await interaction.editReply({
